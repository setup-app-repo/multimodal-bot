...
export function registerBasicHandlers(bot: Bot<BotContext>, deps: RegisterCommandsDeps) {
    ...
    bot.command('start', async (ctx) => {
        ...
        if (!savedLang) {
            ...
            await ctx.reply(t(ctx, 'start_language_welcome'), { reply_markup: KeyboardBuilder.buildLanguageInlineKeyboard(ctx, t) });
            return;
        }
        ...
        await ctx.reply(promoTextStartMd, { reply_markup: KeyboardBuilder.buildMainReplyKeyboard(ctx, t), parse_mode: 'Markdown' });
    });
}
