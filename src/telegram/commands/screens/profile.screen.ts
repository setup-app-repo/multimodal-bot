...
import { DEFAULT_MODEL, getPriceSP, MODELS_SUPPORTING_AUDIO, MODELS_SUPPORTING_FILES, MODELS_SUPPORTING_PHOTOS, PREMIUM_SUBSCRIPTION_COST_SP } from '../../constants';
...
export class ProfileScreen {
    ...
    async build(ctx: BotContext): Promise<ScreenData> {
        ...
        let model = (await redisService.get<string>(`chat:${userId}:model`)) || DEFAULT_MODEL;
        ...
        const balanceLine = t(ctx, 'profile_balance', { balance: spBalance }).replace(/^(?:[^:]+:)/, (m) => `<b>${m}</b>`);
        const premiumLine = t(ctx, 'profile_premium', { status: premiumLabel }).replace(/^(?:[^:]+:)/, (m) => `<b>${m}</b>`);
        const modelLine = t(ctx, 'current_model', { model: modelDisplay }).replace(/^(?:[^:]+:)/, (m) => `<b>${m}</b>`);
        const langLine = t(ctx, 'current_language', { lang: this.getLanguageNameWithoutFlag(ctx, currentLang) }).replace(/^(?:[^:]+:)/, (m) => `<b>${m}</b>`);
