...
import { DEFAULT_MODEL, MODELS_SUPPORTING_FILES, MODELS_SUPPORTING_PHOTOS, MODELS_SUPPORTING_AUDIO, getPriceSP, models, MODEL_INFO } from '../../constants';
...
export class ModelScreen {
    ...
    async buildConnected(ctx: BotContext, modelFromParams?: string): Promise<ScreenData> {
        ...
        const selectedModel = modelFromParams || (await redisService.get<string>(`chat:${userId}:model`)) || DEFAULT_MODEL;
    ...
    async buildSelectionKeyboard(ctx: BotContext): Promise<{ text: string; keyboard: InlineKeyboard }> {
        const { t, subscriptionService, redisService } = this.deps;
        const userId = String(ctx.from?.id);
