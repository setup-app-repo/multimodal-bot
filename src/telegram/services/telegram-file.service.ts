...
export class TelegramFileService {
    ...
    async saveFileMeta(userId: string, meta: { fileId: string; fileName?: string; mimeType: string; fileSize?: number; timestamp?: number }, ttlSeconds: number = 3600): Promise<void> {
        const key = `file:${userId}:${meta.fileId}`;
        const payload = JSON.stringify({ ...meta, timestamp: meta.timestamp ?? Date.now() });
    ...
    async consumeLatestFileAndProcess(userId: string, ctx: BotContext): Promise<string | undefined> {
        try {
            ...
            const fileInfo = JSON.parse(fileInfoStr) as { fileId: string; fileName?: string; mimeType: string };
            const fileId = fileInfo.fileId;

            this.logger.log(`Processing file ${fileInfo.fileName ?? fileId} (${fileInfo.mimeType}) for user ${userId}`);
            ...
            this.logger.error(`Error processing latest file for user ${userId}:`, error as any);
            return undefined;
        }
    }
}
