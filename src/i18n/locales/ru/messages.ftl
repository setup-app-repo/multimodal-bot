...
premium_insufficient_sp = Недостаточно SP: нужно 10, у вас {$balance}. Пополните баланс и попробуйте снова.
...
premium_autorenew_enabled = Автопродление включено ✅\nБудем списывать 10 SP в день продления — {$expires_at}. Отключить можно в Профиль → Премиум.
...
premium_active_text = Действует до: {$expires_at} — + 30 дней(осталось {$days_left} дн.)\nАвтопродление: {$autorenew}\nБаланс: {$balance} SP\n\nПреимущества: скидка на платные запросы, приоритет, повышенные лимиты
premium_extend_30_button = Продлить на 30 дней — 10 SP
premium_autorenew_toggle_button_on = Автопродление: {$on}
premium_autorenew_toggle_button_off = Автопродление: {$off}
switch_on = ВКЛ
switch_off = ВЫКЛ
...
# Кнопка поддержки
help_support_button = 👤 Связаться с поддержкой
...
# Очистка контекста
clear_confirm =  Подтвердите: очистить текущий диалог?
cancel_button = Отмена
context_cleared =  🧹 Контекст очищен.

# Выбор модели
select_model =  🤖 Выберите модель для чата:
select_model_title = Выбери модель (SP/запрос • 🧠 сила ума)
