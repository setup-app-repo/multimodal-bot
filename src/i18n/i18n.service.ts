...
export class I18nService implements II18nService {
  ...
  t(key: string, locale?: string, args?: Record<string, any>): string {
    try {
      ...
      if (!translation || translation === ftlKey) {
        this.logger.warn(`Missing translation for key: ${ftlKey} (original: ${key}, locale: ${targetLocale})`);
      ...
      if (error instanceof Error) {
        this.logger.error(`Error getting translation for key ${key}: ${error.message}`, error.stack);
      } else {
        this.logger.error(`Error getting translation for key ${key} (unknown error type):`, String(error));
      }
  ...
  private normalizeLocale(locale?: string): string {
    ...
    const normalized = locale.toLowerCase().split('-')[0];
    return this.config.supportedLocales.includes(normalized) ? normalized : this.config.defaultLocale;
  }
